...
function ajaxpost(formid, showid, func) {
	...
	if (str.match(/inajax/gi) == null) str += '&inajax=1';
	str = str.replace("social_plugins/", "");
	...
	$(formid).submit();
	//esdTracker.trackIt($(formid).action);
...
function ajaxpost_load() {
	showloading('none');	
	if(is_ie) {
		var tmps = $(ajaxpostHandle[1]).contentWindow.document.documentElement.innerText;
		if (tmps == undefined) {
			tmps = $(ajaxpostHandle[1]).contentWindow.document.documentElement.textContent;
		}
		var sArr = /<!\[CDATA\[([\s\S]+)\]\]>/.exec(tmps);
		if(sArr && sArr[1]){
			var s=sArr[1].replace(/^\s+|\s+$/g,'');
		}else{
			var s=tmps;
		} 
	} else {
	...
	if(!evaled && (typeof ajaxerror == 'undefined' || !ajaxerror)) {
		...
		if (window.location.href.indexOf("do.php?ac=3c7495bde72b124b806ed64fb89785db") > -1) {
			var strRef = getParameterByName('ref');
			if (strRef == "") {
				setTimeout("location.href='/space.php?do=home';", 2000);
			} else {
				setTimeout("location.href='" + strRef + "';", 2000);
			}
		}
		
		if(!evaled)evalscript(s);
		//setMenuPosition($(ajaxpostHandle[0]).ctrlid, 0);
		if(typeof ajax_post_auto_close_disable == 'undefined'){
