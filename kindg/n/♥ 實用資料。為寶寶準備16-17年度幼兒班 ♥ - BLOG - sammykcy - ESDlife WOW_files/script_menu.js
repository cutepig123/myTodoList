...
function showMenu(ctrlid, click, offset, duration, timeout, layer, showid, maxh, drag) {
	...
	if(is_ie && is_ie < 7) {
		...
		var ie_top=menuobj.style.top;
		if(ie_top < 0)
			ie_top = 0;
		jsmenu['iframe'][layer].style.top = ie_top;
...
function setMenuPosition(showid, offset) {
	...
	if(showobj) {
		...
		if(offset != -1) {
			...
			//解決Chrome的無法讀取documentElement的scrollTop而無法準確定位 by Jack 20130318
			if(document.body.scrollTop == 0){
				var divtop = document.documentElement.scrollTop + (document.documentElement.clientHeight-menuobj.h)/2;
			}else{
				var divtop = document.body.scrollTop + (document.documentElement.clientHeight-menuobj.h)/2;
			}
			//End
